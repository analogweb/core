...
public class AnnotationInvocationMetadataFactory implements InvocationMetadataFactory {
    ...
    @Override
    public Collection<InvocationMetadata> createInvocationMetadatas(ApplicationProperties properties,
        ...
        packageNames.addAll(properties.getComponentPackageNames() == null ? Collections.<String> emptyList()
    ...
    InvocationMetadata createInvocationMetadata(Class<?> invocationClass, Method invocationMethod) {
        ...
        if (typePathMapping != null && methodPathMapping != null) {
            return new DefaultInvocationMetadata(invocationClass, invocationMethod.getName(),
                    ...
                    relativeRequestPath(invocationClass, invocationMethod, typePathMapping, methodPathMapping));
        } else {
            return null;
        }
    }

    protected RequestPathMetadata relativeRequestPath(Class<?> invocationClass, Method invocationMethod,
            Route typePathMapping, Route methodPathMapping) {
        ...
        return newRequestPathDefinition(editedRoot, editedPath, resolveRequestMethods(invocationMethod));
    }
