...
public class Redirect implements Renderable {
	...
	protected void sendRedirect(RequestContext context,
			ResponseContext response, URI encodedPath, int responseCode)
			throws IOException {
	...
	protected String getParametarizedPath() {
		...
		if (getParameters().isEmpty() == false) {
			...
			Iterator<Entry<String, String>> entriesIterator = entries
					.iterator();
	...
	public boolean equals(final Object o) {
		if (o instanceof Redirect) {
			return getParametarizedPath().equals(
					((Redirect) o).getParametarizedPath());
		}
		return false;
	}
