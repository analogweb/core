...
class JulLog extends AbstractLog {
    ...
    private static final String DEFAULT_LOG_CONFIG_IMPLEMENTATION = AbstractLog.class.getPackage().getName()
            + ".JulLogConfigImpl";
    ...
    private JulLogConfig configureViaRuntimeClass(ClassLoader classLoader) {
        Class<?> clazz = ClassUtils.forNameQuietly(DEFAULT_LOG_CONFIG_IMPLEMENTATION, classLoader);
        if (clazz != null) {
