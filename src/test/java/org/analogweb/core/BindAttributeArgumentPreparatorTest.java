...
public class BindAttributeArgumentPreparatorTest {
	...
	public void testPrepare() {
		...
		final Method method = ReflectionUtils.getMethodQuietly(
				StubResource.class, "doSomething", argumentTypes);
		...
		when(
				handler.resolveValue(context, metadata, "foo",
						argumentTypes[0], anns[0])).thenReturn("foo!");
		when(
				typeMapper.mapToType(TypeMapper.class, "foo!", String.class,
						new String[0])).thenReturn("foo!");
		when(
				handler.resolveValue(context, metadata, "baa",
						argumentTypes[1], anns[1])).thenReturn("baa!");
		when(
				typeMapper.mapToType(TypeMapper.class, "baa!", String.class,
	...
	public void testScope() {
		final Class<?>[] argumentTypes = new Class<?>[]{String.class};
		final Method method = ReflectionUtils.getMethodQuietly(
				StubResource.class, "doScope", argumentTypes);
		final Annotation[][] anns = method.getParameterAnnotations();
		AttributesHandler handler = mock(AttributesHandler.class);
		...
		when(metadata.resolveMethod()).thenReturn(method);
		when(
				handler.resolveValue(context, metadata, "baz",
						argumentTypes[0], anns[0])).thenReturn("foo!");
		when(
				typeMapper.mapToType(TypeMapper.class, "foo!", String.class,
						new String[0])).thenReturn("foo!!");
	...
	public void testInclusionScope() {
		final Class<?>[] argumentTypes = new Class<?>[]{String.class};
		final Method method = ReflectionUtils.getMethodQuietly(
				StubResource.class, "doScopeVariable", argumentTypes);
		final Annotation[][] anns = method.getParameterAnnotations();
		AttributesHandler handler = mock(AttributesHandler.class);
		when(handlers.findRequestValueResolver(Foo.class)).thenReturn(handler);
		when(metadata.getArgumentTypes()).thenReturn(argumentTypes);
	...
	public void testPrepareWithCoveredAnnotation() {
		final Class<?>[] argumentTypes = new Class<?>[]{String.class};
		final Method method = ReflectionUtils.getMethodQuietly(
				StubResource.class, "doWithCustomAnnotation", argumentTypes);
		final Annotation[][] anns = method.getParameterAnnotations();
		AttributesHandler handler = mock(AttributesHandler.class);
		...
		when(
				handler.resolveValue(context, metadata, "foo",
						argumentTypes[0], anns[0])).thenReturn("boo!");
		when(
				typeMapper.mapToType(SomeTypeMapper.class, "boo!",
	...
	public void testPrepareWithFormat() {
		...
		final Method method = ReflectionUtils.getMethodQuietly(
				StubResource.class, "doWithFormat", argumentTypes);
		final Annotation[][] anns = method.getParameterAnnotations();
		AttributesHandler handler = mock(AttributesHandler.class);
		when(handlers.findDefaultRequestValueResolver()).thenReturn(handler);
		when(metadata.getArgumentTypes()).thenReturn(argumentTypes);
		...
		when(
				handler.resolveValue(context, metadata, "foo",
						argumentTypes[0], anns[0])).thenReturn("100,000");
		BigDecimal expected = new BigDecimal(100000L);
		when(
				typeMapper.mapToType(TypeMapper.class, "100,000",
						BigDecimal.class, new String[]{"###,###"})).thenReturn(
				expected);
		Object actual = preparator.prepareInvoke(args, metadata, context,
				typeMapper, handlers);
		assertSame(actual, ApplicationProcessor.NO_INTERRUPTION);
	...
	private static final class StubResource {
		...
		@Route
		public String doWithFormat(
				@As("foo") @Formats("###,###") BigDecimal amount) {
			return "do nothing!";
		}
	}
