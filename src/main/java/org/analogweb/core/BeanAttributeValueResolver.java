...
public class BeanAttributeValueResolver implements RequestValueResolver, ModulesAware {
    ...
    @Override
    public Object resolveValue(RequestContext context, InvocationMetadata metadata, String key, Class<?> requiredType,
            Annotation[] parameterAnnotations) {
        Object beanInstance = instanticate(requiredType,
                AnnotationUtils.findAnnotation(Resolver.class, parameterAnnotations), metadata, context,
                getRequestValueResolvers(), parameterAnnotations);
        if (beanInstance != null) {
            for (Field field : requiredType.getDeclaredFields()) {
                Object convertedValue = AnnotatedArguments.resolveArguent(field.getName(), field.getAnnotations(),
                        field.getType(), context, metadata, getTypeMapperContext(), getRequestValueResolvers());
    ...
    private Object instanticate(Class<?> clazz, Resolver resolverAnn, InvocationMetadata metadata,
            RequestContext context, RequestValueResolvers resolvers, Annotation[] parameterAnnotations) {
        if (resolverAnn != null) {
            ...
            if (resolver != null
                ...
                return resolver.resolveValue(context, metadata, StringUtils.EMPTY, clazz, parameterAnnotations);
            }
        }
