...
public class DefaultApplicationPropertiesTest {
	...
	public void testUsingDefaultCreator() throws IOException {
		...
		Collection<String> actualPackageNames = properties
				.getComponentPackageNames();
		assertThat(actualPackageNames.size(), is(1));
		assertThat(actualPackageNames.contains(Application.class.getPackage()
				.getName()), is(true));
		assertThat(properties.getTempDir().getPath(), startsWith(new File(
				SystemProperties.tmpDir() + SystemProperties.fileSeparator()
						+ Application.class.getCanonicalName()).getPath()));
	}

	@Test
	public void testUsingArgsCreator() throws IOException {
		properties = DefaultApplicationProperties.properties(new String[]{
				"arg1", "arg2=arg3", "analogweb.tmpdir=/foo/baa/baz"});
		Collection<String> actualPackageNames = properties
				.getComponentPackageNames();
		assertThat(actualPackageNames.size(), is(1));
		assertThat(actualPackageNames.contains(Application.class.getPackage()
				.getName()), is(true));
		assertThat(properties.getTempDir().getPath(), is(new File(
				"/foo/baa/baz").getPath()));
	...
	public void testUsingConfiguredDefaultCreator() throws IOException {
		...
		properties = DefaultApplicationProperties.properties(packageNames,
				tempDirectoryPath, locale);
		Collection<String> actualPackageNames = properties
				.getComponentPackageNames();
		assertThat(actualPackageNames.size(), is(3));
		assertThat(actualPackageNames.containsAll(Arrays.asList("foo.baa",
				"baz.boo", "org.analogweb")), is(true));
		assertThat(
				properties.getTempDir().getPath(),
				startsWith(new File(dir.getPath()
	...
	@Test
	public void testUsingConfiguredDefaultCreatorWithEmptyPackageNames()
			throws IOException {
		File dir = folder.newFolder();
		String packageNames = "";
		String tempDirectoryPath = dir.getPath();
		...
		assertThat(properties.getComponentPackageNames().iterator().next(),
				is("org.analogweb"));
