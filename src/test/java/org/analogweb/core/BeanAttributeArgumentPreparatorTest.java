...
public class BeanAttributeArgumentPreparatorTest {
    ...
    public void test() throws Exception {
        ...
        Annotation[] anns = SomeBean.class.getDeclaredField("name").getAnnotations();
        when(resolver.resolveValue(context, metadata, "name", String.class, anns)).thenReturn("Name");
        when(resolver.resolveValue(context, metadata, "birthDay", Date.class, anns)).thenReturn("2013-01-01");
        when(resolver.resolveValue(context, metadata, "number", Integer.class, anns)).thenReturn("5");
        Date expectedDate = new Date();
        when(converters.mapToType(eq(TypeMapper.class), eq("Name"), eq(String.class), isA(String[].class)))
                .thenReturn("Name");
        when(converters.mapToType(eq(TypeMapper.class), eq("2013-01-01"), eq(Date.class), isA(String[].class)))
                .thenReturn(expectedDate);
        when(converters.mapToType(eq(TypeMapper.class), eq("5"), eq(Integer.class), isA(String[].class))).thenReturn(5);
        Method method = Resource.class.getDeclaredMethod("doSomething", SomeBean.class, String.class);
        SomeBean actual = (SomeBean) preparator.resolveValue(context, metadata, "", SomeBean.class,
    ...
    public void testWithResolver() throws Exception {
        ...
        Annotation[] anns = SomeBean.class.getDeclaredField("name").getAnnotations();
        when(beanResolver.resolveValue(context, metadata, StringUtils.EMPTY, SomeBean.class, anns))
        ...
        when(converters.mapToType(eq(TypeMapper.class), eq("2013-01-01"), eq(Date.class), isA(String[].class)))
                .thenReturn(expectedDate);
        when(converters.mapToType(eq(TypeMapper.class), eq("5"), eq(Integer.class), isA(String[].class))).thenReturn(5);
        Method m = Resource.class.getDeclaredMethod("doAnything", SomeBean.class, String.class);
        SomeBean actual = (SomeBean) preparator.resolveValue(context, metadata, "", SomeBean.class, m.getAnnotations());
        assertThat(actual.getName(), is("Name"));
        assertThat(actual.getBirthDay(), is(expectedDate));
        assertThat(actual.getNumber(), is(5));
    }
    ...
    public static class Resource {
        ...
        public String doAnything(@Bean @Resolver(value = SomeBeanResolver.class) SomeBean bean, @Param String comment) {
            return "something!";
        }
    }
    ...
    public static class SomeBeanResolver implements RequestValueResolver {
        ...
        @Override
        public Object resolveValue(RequestContext requestContext, InvocationMetadata metadata, String key,
