 ...
 * @author snowgoose
 */
public class DefaultTypeMapperContext
		implements
			TypeMapperContext,
	...
	public <T> T mapToType(Class<? extends TypeMapper> typeMapperClass,
			Object from, Class<T> requiredType, String[] formats) {
		Assertion.notNull(requiredType, "RequiredType");
		log.log(Markers.VARIABLE_ACCESS, "DC000001", from, requiredType,
				formats);
		...
		if (typeMapper != null) {
			...
			return (T) getDefaultTypeMapper().mapToType(from, requiredType,
					formats);
		}
	}
		...
		if (from.getClass().isArray() && ArrayUtils.isNotEmpty((Object[]) from)
			...
			if (requiredType.isAssignableFrom(result.getClass())) {
				return (T) result;
					...
					&& requiredType.getComponentType().isAssignableFrom(
							result.getClass())) {
				return (T) from;
			}
		}
	...
	protected <T> T handleCollection(Class<T> requiredType, Object from) {
		...
		if (from instanceof Collection) {
			...
			if (fromCollection.isEmpty() == false
					&& requiredType.isAssignableFrom((result = fromCollection
