...
public class RequestContextValueResolverTest {
    ...
    public void testResolved() {
        Object actual = resolver.resolveValue(requestContext, metadata, "", RequestPath.class, new Annotation[0]);
        assertThat((RequestPath) actual, is(path));
        actual = resolver.resolveValue(requestContext, metadata, "", MediaType.class, new Annotation[0]);
        assertThat((MediaType) actual, is(type));
        actual = resolver.resolveValue(requestContext, metadata, "", Headers.class, new Annotation[0]);
        assertThat((Headers) actual, is(headers));
        actual = resolver.resolveValue(requestContext, metadata, "", Cookies.class, new Annotation[0]);
        assertThat((Cookies) actual, is(cookies));
    }

    @Test
    public void testNotResolved() {
        Object actual = resolver.resolveValue(requestContext, metadata, "", String.class, new Annotation[0]);
        assertThat(actual, is(nullValue()));
        actual = resolver.resolveValue(requestContext, metadata, "", InputStream.class, new Annotation[0]);
        assertThat(actual, is(nullValue()));
        actual = resolver.resolveValue(requestContext, metadata, "", null, new Annotation[0]);
        assertThat(actual, is(nullValue()));
    }
}
