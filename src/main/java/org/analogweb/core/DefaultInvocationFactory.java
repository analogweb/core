...
public class DefaultInvocationFactory implements InvocationFactory {
    ...
    public Invocation createInvocation(ContainerAdaptor instanceProvider, InvocationMetadata metadata,
        ...
        if (invocationInstance == null) {
            invocationInstance = resolveByDefault(metadata, context, responseContext, converters, handlers);
            if (invocationInstance == null) {
    ...
    protected Object resolveByDefault(InvocationMetadata metadata, RequestContext context,
            ResponseContext responseContext, TypeMapperContext converters, RequestValueResolvers handlers) {
        ...
        for (int index = 0, limit = types.length; index < limit; index++) {
            ...
            argValues.add(AnnotatedArguments.resolveArguent(ann, type, context, metadata, converters, handlers));
        }
        return ReflectionUtils.getInstanceQuietly(firstConstructor, argValues.toArray(new Object[argValues.size()]));
    }

    protected Object resolveInvocationInstance(ContainerAdaptor instanceProvider, InvocationMetadata metadata,
            RequestContext context) throws UnresolvableInvocationException {
        Object invocationInstance = instanceProvider.getInstanceOfType(metadata.getInvocationClass());
