...
public class TextTest {
	...
	public void testDefaultText() throws Exception {
		...
		assertThat(actual.resolveContentType(), is("text/plain; charset="
				+ charset));
		assertThat(actual.getCharsetAsText(), is(Charset.defaultCharset()
				.displayName()));
	...
	public void testXMLText() throws Exception {
		...
		assertThat(actual.resolveContentType(), is("text/xml; charset="
				+ charset));
		...
		assertThat(new String(entity), is(responseText));
		verify(headers)
				.putValue("Content-Type", "text/xml; charset=" + charset);
	}
	...
	public void testJSONText() throws Exception {
		...
		final Text actual = Text.with(responseText).typeAs("application/json")
				.withCharset(charset);
		assertThat(actual.resolveContentType(), is("application/json; charset="
				+ charset));
		Headers headers = mock(Headers.class);
		when(response.getResponseHeaders()).thenReturn(headers);
		Response r = actual.render(context, response);
	...
	public void testMultibyteText() throws Exception {
		...
		assertThat(actual.resolveContentType(),
				is("text/plain; charset=Shift-JIS"));
