...
public class DefaultInvokerTest {
    ...
    public void testInvoke() {
        ...
        when(processor.prepareInvoke(args, metadata, request, typeMapper, handlers))
                .thenReturn(ApplicationProcessor.NO_INTERRUPTION);
        doNothing().when(processor).postInvoke("foo is something!!", args, metadata, request, handlers);
    ...
    public void testInvokeWithException() {
        ...
        when(processor.prepareInvoke(args, metadata, request, typeMapper, handlers))
                .thenReturn(ApplicationProcessor.NO_INTERRUPTION);
        when(processor.processException(isA(IllegalArgumentException.class), eq(request), eq(args), eq(metadata)))
                .thenReturn(ApplicationProcessor.NO_INTERRUPTION);
