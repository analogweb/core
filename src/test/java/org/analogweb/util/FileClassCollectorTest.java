...
public class FileClassCollectorTest {
	...
	private static final String PACKAGE_NAME = FileClassCollectorTest.class
	...
	public void testCollect() throws Exception {
		...
		assertContainsInstanceOfTypes(collectedClasses, new Class<?>[]{
				Foo.class, Baa.class, Baz.class, Boo.class, Bee.class});
	}
	...
	public void testCollectWithNullPackageName() throws Exception {
		Collection<Class<?>> result = collector
				.collect(null, null, classLoader);
		assertTrue(result.isEmpty());
	}

	@Test
	public void testCollectWithNullResource() throws Exception {
		Collection<Class<?>> result = collector.collect(PACKAGE_NAME, null,
				null);
		assertTrue(result.isEmpty());
	}
	...
	public void testCollectWithInvalidPackageName() throws Exception {
		Collection<Class<?>> result = collector.collect("invalid.package.name",
				null, classLoader);
		assertTrue(result.isEmpty());
	}

	private void assertContainsInstanceOfTypes(Collection<?> actual,
			Class<?>... expectedClasses) {
		for (Class<?> expected : expectedClasses) {
			for (Object obj : actual) {
				if (expected.equals((obj instanceof Class<?>) ? obj : obj
						.getClass())) {
					return;
				}
			}
			fail(String.format(
					"actual collection not contains instance of class %s",
					expected));
		}
	}

	private URL getPackageURL(String packageName) {
		return ResourceUtils.findPackageResources(packageName, classLoader)
				.get(0);
	}
}
