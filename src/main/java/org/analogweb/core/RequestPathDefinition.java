...
public class RequestPathDefinition extends AbstractRequestPathMetadata {
    ...
    public boolean match(RequestPath requestPath) {
        return matchWildCard(requestPath, getActualPath()) || matchPlaceHolder(requestPath)
    ...
    private boolean containsRequestMethod(RequestPath other) {
        if (getRequestMethods().contains(other.getRequestMethod()) == false) {
            throw new RequestMethodUnsupportedException(this, getRequestMethods(), other.getRequestMethod());
        } else {
            return true;
        }
    }
