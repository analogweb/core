...
public class AcceptableTest {
    ...
    public void testRenderSwitchedAcceptable() throws Exception {
        final Member m = new Member("snowgoose", 34);
        when(context.getRequestHeaders()).thenReturn(headers);
        when(headers.getValues("Accept")).thenReturn(Arrays.asList("text/x-dvi", "image/png", "application/json"));
    ...
    public void testSelectAcceptableOne() throws Exception {
        final Member m = new Member("snowgoose", 34);
        when(context.getRequestHeaders()).thenReturn(headers);
        when(headers.getValues("Accept"))
                .thenReturn(Arrays.asList("text/html", "text/x-dvi", "image/png", "application/json"));
        final Renderable jsonResponse = mock(Renderable.class);
        final Renderable htmlResponse = mock(Renderable.class);
        Acceptable acceptable = Acceptable.as(m).map(jsonResponse, "application/json").map(htmlResponse, "text/html");
    ...
    public void testComparator() {
        final List<String> accepts = Arrays.asList(" text/plain", " application/*;level=1", " application/json", " */*",
                " text/html;q=1", " text/*");
