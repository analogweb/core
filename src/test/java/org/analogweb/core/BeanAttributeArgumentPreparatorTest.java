...
public class BeanAttributeArgumentPreparatorTest {
	...
	public void test() throws Exception {
		...
		when(resolvers.findRequestValueResolver(ParameterValueResolver.class))
				.thenReturn(resolver);
		when(resolver.resolveValue(context, metadata, "name", String.class))
				.thenReturn("Name");
		when(resolver.resolveValue(context, metadata, "birthDay", Date.class))
				.thenReturn("2013-01-01");
		when(resolver.resolveValue(context, metadata, "number", Integer.class))
				.thenReturn("5");
		...
		preparator.prepareInvoke(Resource.class.getDeclaredMethod(
				"doSomething", SomeBean.class, String.class), args, metadata,
				context, converters, resolvers);

		ArgumentCaptor<SomeBean> captor = ArgumentCaptor
	...
	public void testWithResolver() throws Exception {
		...
		when(
				beanResolver.resolveValue(context, metadata, StringUtils.EMPTY,
						SomeBean.class)).thenReturn(resolvedBean);
		when(resolver.resolveValue(context, metadata, "name", String.class))
				.thenReturn("Name");
		when(resolver.resolveValue(context, metadata, "birthDay", Date.class))
				.thenReturn("2013-01-01");
		...
		when(
				converters.mapToType(eq(TypeMapper.class), eq("5"),
						eq(Integer.class), isA(String[].class))).thenReturn(5);
		preparator.prepareInvoke(Resource.class.getDeclaredMethod("doAnything",
				SomeBean.class, String.class), args, metadata, context,
