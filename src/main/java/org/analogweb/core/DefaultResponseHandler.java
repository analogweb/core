...
public class DefaultResponseHandler implements ResponseHandler {
    ...
    @Override
    public Response handleResult(Object result, InvocationMetadata metadata, RenderableResolver renderableResolver,
        ...
        Renderable resolved = renderableResolver.resolve(result, metadata, context, response);
        ...
        if (resolved instanceof RenderableHolder) {
            ...
            if (renderable != null) {
                resultFormatter = finder.findResponseFormatter(renderable.getClass());
            }
        } else {
        ...
        return handleResultInternal(resolved, metadata, renderableResolver, resultFormatter, context, response,
                exceptionHandler, finder);
    }

    protected Response handleResultInternal(Renderable result, InvocationMetadata metadata,
            RenderableResolver renderableResolver, ResponseFormatter resultFormatter, RequestContext context,
            ResponseContext response, ExceptionHandler exceptionHandler, ResponseFormatterFinder finder)
        ...
        try {
            ...
            return handleResult(exceptionHandler.handleException(e), metadata, renderableResolver, context, response,
                    exceptionHandler, finder);
        }
    }
}
