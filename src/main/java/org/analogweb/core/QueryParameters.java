...
public class QueryParameters implements Parameters {
    ...
    protected Map<String, String[]> extractEncodedParams(String encoded, Charset charset, char separator)
            throws IOException {
        ...
        for (String token : tokenized) {
            ...
            if (idx < 0) {
                ...
                if (map.containsKey(key)) {
                    values = map.get(key);
                    values = ArrayUtils.add(String.class, URLDecoder.decode(token.substring(idx + 1), charset.name()),
                            values);
                } else {
                    values = ArrayUtils.newArray(URLDecoder.decode(token.substring(idx + 1), charset.name()));
                }
                map.put(key, values);
            }
        }
