	...
	public void testPrepare() {
		...
		when(
				handler.resolveValue(context, metadata, "foo",
						argumentTypes[0], anns[0])).thenReturn(
				"foo!");
		when(
		...
		when(
				handler.resolveValue(context, metadata, "baa",
						argumentTypes[1], anns[1])).thenReturn(
				"baa!");
		when(
				typeMapper.mapToType(TypeMapper.class, "baa!", String.class,
						new String[0])).thenReturn("baa!");
		Object actual = preparator.prepareInvoke(method, args, metadata,
				context, typeMapper, handlers);
