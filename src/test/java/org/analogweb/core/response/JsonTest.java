...
public class JsonTest {
	...
	public void testSingleObject() throws Exception {
		...
		assertThat(json.resolveContentType(), is("application/json; charset="
				+ charset));
		...
		assertThat(actual,
				is("{\"age\": 33,\"birthDay\": " + birthDay.getTime()
						+ ",\"name\": \"foo\"}"));
	...
	public void testList() throws Exception {
		...
		List<Simple> beans = Arrays.asList(new Simple("foo", 33, birthDay),
		...
		assertThat(actual,
				is("{[" + "{\"age\": 33,\"birthDay\": " + birthDay.getTime()
						+ ",\"name\": \"foo\"},"
						+ "{\"age\": 32,\"birthDay\": " + birthDay2.getTime()
						+ ",\"name\": \"baa\"}" + "]}"));
	}
	...
	public void testManyList() throws Exception {
		...
		assertThat(actual, is("{\"boo\": true,\"simples\": ["
				+ "{\"age\": 33,\"birthDay\": " + birthDay.getTime()
				+ ",\"name\": \"foo\"}," + "{\"age\": 32,\"birthDay\": "
	...
	public void testManyArray() throws Exception {
		...
		byte[] body = (byte[]) r.getEntity().entity();
		String actual = new String(body, "UTF-8");
		assertThat(actual, is("{\"id\": \"01\",\"simples\": ["
				+ "{\"age\": 33,\"birthDay\": " + birthDay.getTime()
				+ ",\"name\": \"foo\"}," + "{\"age\": 32,\"birthDay\": "
				+ birthDay2.getTime() + ",\"name\": \"baa\"}" + "]}"));
	}
	...
	public void testContainsNull() throws Exception {
		...
		String actual = new String(body, charset);
		assertThat(actual,
				is("{\"age\": 33,\"birthDay\": null,\"name\": \"foo\"}"));
	}
	...
	static class ManyList implements Serializable {
		...
		ManyList() {
			...
			try {
				simples.add(new Simple("foo", 33, new SimpleDateFormat(
						"yyyyMMdd").parse("20110420")));
				simples.add(new Simple("baa", 32, new SimpleDateFormat(
						"yyyyMMdd").parse("20100712")));
	...
	static class ManyArray implements Serializable {
		...
		ManyArray() {
			...
			try {
				simples[0] = new Simple("foo", 33, new SimpleDateFormat(
						"yyyyMMdd").parse("20110420"));
				simples[1] = new Simple("baa", 32, new SimpleDateFormat(
						"yyyyMMdd").parse("20100712"));
			} catch (ParseException e) {
				// nop.
			}
