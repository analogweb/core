...
public class IntrgrationTest {
    ...
    public void testFound() {
        ...
        assertThat(actual.getResponseHeader().get("Content-Type").get(0), is("text/plain; charset=UTF-8"));
    }
    ...
    public void testPostFormToBean() {
        ...
        ResponseResult actual = app.request("/helloBean", "POST",
                Maps.newHashMap("Content-Type", Arrays.asList("application/x-www-form-urlencoded")), body);
        assertThat(actual.getStatus(), is(200));
        assertThat(actual.toBody(), is("foo"));
        assertThat(actual.getResponseHeader().get("Content-Type").get(0), is("text/plain; charset=UTF-8"));
    }
