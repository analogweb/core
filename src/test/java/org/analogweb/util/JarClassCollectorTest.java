...
public class JarClassCollectorTest {
	...
	private static final String PACKAGE_NAME = JarClassCollectorTest.class
			.getPackage().getName() + ".classcollector";
	...
	public void testCollect() throws Exception {
		final URL resource = Thread
				.currentThread()
				.getContextClassLoader()
				.getResource(
						getClass().getCanonicalName().replace('.', '/')
								+ ".jar");
		smallClassLoader = AccessController
		...
		Collection<String> scannedClassNames = classNames(collector.collect(
				"a.b.c",
				ResourceUtils.findPackageResources("a.b.c", smallClassLoader)
						.get(0), smallClassLoader));
		...
		scannedClassNames = classNames(collector.collect("a.b.d", ResourceUtils
				.findPackageResources("a.b.d", smallClassLoader).get(0),
				smallClassLoader));
		assertThat(scannedClassNames.size(), is(2));
		...
		scannedClassNames = classNames(collector.collect("a.b.e", ResourceUtils
				.findPackageResources("a.b.e", smallClassLoader).get(0),
				smallClassLoader));
	...
	public void testCollectWithDirectory() throws Exception {
		Collection<Class<?>> collectedClasses = collector.collect(PACKAGE_NAME,
				ResourceUtils.findPackageResources(PACKAGE_NAME, classLoader)
						.get(0), classLoader);
	...
	public void testCollectWithInvalidPackageName() throws Exception {
		Collection<Class<?>> collectedClasses = collector.collect(
				"invalid.package.name", null, classLoader);
		assertThat(collectedClasses.isEmpty(), is(true));
	}
	...
	public void testCollectWithNullPackageName() throws Exception {
		...
		Collection<Class<?>> result = collector.collect(null, ResourceUtils
				.findPackageResources("a.b.c", smallClassLoader).get(0),
				smallClassLoader);
	...
	public void testCollectWithNullResourceURL() throws Exception {
		Collection<Class<?>> result = collector.collect("a.b.c", null,
				classLoader);
